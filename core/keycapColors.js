...
// keyviz writes CSS colours: #RRGGBB, or #RRGGBBAA with the alpha LAST. Qt
// reads an 8-digit hex the other way round (#AARRGGBB), so both directions are
// converted explicitly -- otherwise "#ffffff99" (keyviz's group panel, white at
// 60%) silently turns opaque.
// Components are 0..1, matching what Qt.rgba() takes. Returns null for anything
// that is not a hex colour, so the caller can fall back to Qt.color().
function cssToRgba(value) {
    if (typeof value !== "string") return null;
    let hex = value.trim();
    if (/^#[0-9a-f]{4}$/i.test(hex)) hex = "#" + hex.slice(1).split("").map(c => c + c).join("");
    const rgb = /^#[0-9a-f]{6}$/i.test(hex);
    const rgba = /^#[0-9a-f]{8}$/i.test(hex);
    if (!rgb && !rgba) return null;
    return {
        r: parseInt(hex.slice(1, 3), 16) / 255,
        g: parseInt(hex.slice(3, 5), 16) / 255,
        b: parseInt(hex.slice(5, 7), 16) / 255,
        a: rgba ? parseInt(hex.slice(7, 9), 16) / 255 : 1
    };
}

// The inverse of cssToRgba: components in, CSS hex out. Alpha is dropped while
// it is fully opaque, so a colour the user never made translucent stays on the
// six-digit form keyviz writes.
function toCss(r, g, b, a) {
    const pair = v => ("0" + Math.round(Math.max(0, Math.min(1, v)) * 255).toString(16)).slice(-2);
    const rgb = "#" + pair(r) + pair(g) + pair(b);
    return (a === undefined || a === null || a >= 1) ? rgb : rgb + pair(a);
}

function cssColor(value) {
    if (typeof value !== "string") return value;
    const c = cssToRgba(value);
    return c ? Qt.rgba(c.r, c.g, c.b, c.a) : Qt.color(value);
}
